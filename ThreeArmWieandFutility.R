#' @title Simulation of a series of three-arm trials with an interim analysis based on Wieand rule.
#' @author Mohamed Amine BAYAR
#' \code{SimulateThreeArmWieandFutility} simulates a series of three-arm trials with an interim analysis based on Wieand rule.
#' 
...
SimulateThreeArmWieandFutility  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      end.s1         <- ifelse((((pvalue1.s1 >= 0.5) & (pvalue2.s1 >= 0.5)) |
                                  ((pvalue1.s1 >= 0.5) & (pvalue12.s1 >= 0.5)) |
                                  ((pvalue2.s1 >= 0.5) & (pvalue12.s1 >= 0.5)) |
                                  ((pvalue12.s1 >= 0.5))), 1, 0)  
...
results <- SimulateThreeArmWieandFutility(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
