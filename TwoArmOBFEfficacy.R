#' @title Simulation of a series of two-arm trials with an interim analysis with a stopping rule for efficacy based on O'Brien-Fleming spending function.
#' @author Mohamed Amine BAYAR
#' \code{SimulateTwoArmOBFEfficacy} simulates a series of two-arm trials with an interim analysis with a stopping rule for efficacy based on O'Brien-Fleming spending function.
#' 
...
SimulateTwoArmOBFEfficacy  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      # ----- HYPOTHESES TESTING  -----
      # -----  ----- FOR THE INTERIM ANALYSIS -----  -----
      # Interim analysis for efficacy with alpha-spending function
      end.ia        <- ifelse(pvalue.ia <= pvalue.efficacy.ia, 1, 0)  # Compare it to critical p-value for efficacy
      lambda.obs.ia <- lambdaE.obs.ia  # if the trial stops for efficacy, the observed hazard rate is the hazard rate from the experimental group, estimated from the sumilated data
      lambda.ia     <- lambdaE  # The hazard rate of the experimental group is selected
...
results <- SimulateTwoArmOBFEfficacy(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
