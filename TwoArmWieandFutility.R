#' @title Simulation of a series of two-arm trials with an interim analysis based on Wieand rule.
#' @author Mohamed Amine BAYAR
#' \code{SimulateTwoArmWieandFutility} simulates a series of two-arm trials with an interim analysis based on Wieand rule.
#' 
...
test.type  <- 4  # 4=two-sided, asymmetric, beta-spending with non-binding lower bound, The programs is set so as not to perform an efficacy IA 
hr0        <- 1  # Hazard ratio experimental/control under the null hypothesis
ratio      <- 1  # Randomization ratio experimental/control
sided      <- 1  # 1-sided testing
ia.type    <- "Wieand"

SimulateTwoArmWieandFutility  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      # ----- HYPOTHESES TESTING  -----
      # -----  ----- FOR THE INTERIM ANALYSIS -----  -----
      # Interim analysis for futility with wieand rule
      end.ia        <- ifelse(hr.obs.ia >= 1, 1, 0)  # If hazard rate >= 1 then stop the trial for futility (Wieand)
      lambda.obs.ia <- lambdaC.obs.ia  # if the trial stops for futility, the observed hazard rate is the hazard rate from the control group, estimated from the sumilated data
      lambda.ia     <- lambdaC  # The hazard rate of the control group is selected
      ...
      # -----  ----- FOR THE FINAL ANALYSIS -----  -----
      # Interim analysis for futility with wieand rule
      # If the trial did not stop for futility, the final analysis is performed as if no interim analysis was performed (ia="none")
...
results <- SimulateTwoArmWieandFutility(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
