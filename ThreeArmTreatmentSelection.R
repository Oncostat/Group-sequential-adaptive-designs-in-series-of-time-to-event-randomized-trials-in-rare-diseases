#' @title Simulation of a series of three-arm trials with only treatment selection at interim (treat.selection).
#' @author Mohamed Amine BAYAR
#' \code{SimulateThreeArmTreatmentSelection} simulates a series of three-arm trials with only treatment selection at interim (treat.selection).
#' 
...
SimulateThreeArmTreatmentSelection  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      end.s1         <- 0  # No IA never stop the trial at interim - Just select the treatment with the smallest - old code: ifelse(min(c(combn(p1,1)))>threshold, 1, 0)
      
      # If the trial stops at the end of the first stage (never the case here), the time from the series initiation is incremented with the first stage ending time,
      ...
      # ----- IF THE TRIAL STOPPED AT INTERIM (FOR FUTILITY) never the case here either for efficacy or for futility -----        
      lambda.s1     <- lambdaC
...
results <- SimulateThreeArmTreatmentSelection(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
