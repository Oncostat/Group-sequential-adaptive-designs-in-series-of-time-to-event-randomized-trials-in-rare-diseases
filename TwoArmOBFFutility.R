#' @title Simulation of a series of two-arm trials with an interim analysis with a stopping rule for futility based on O'Brien-Fleming spending function.
#' @author Mohamed Amine BAYAR
#' \code{SimulateTwoArmOBFFutility} simulates a series of two-arm trials with an interim analysis with a stopping rule for futility based on O'Brien-Fleming spending function.
#' 
...
SimulateTwoArmOBFFutility  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      # ----- HYPOTHESES TESTING  -----
      # -----  ----- FOR THE INTERIM ANALYSIS -----  -----
      # Interim analysis for futility with beta-spending non binding function
      end.ia        <- ifelse(pvalue.ia >= pvalue.futility.ia, 1, 0)  # Compare it to critical p-value for futility
      ...
      # -----  ----- FOR THE FINAL ANALYSIS -----  -----
      # Interim analysis for futility with non-binding beta-spendingfunction
...
results <- SimulateTwoArmOBFFutility(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
