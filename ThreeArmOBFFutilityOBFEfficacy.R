#' @title Simulation of a series of three-arm trials with an interim analysis with a stopping rule for futility and efficacy based on O'Brien-Fleming spending function.
#' @author Mohamed Amine BAYAR
#' \code{SimulatethreeArmOBFFutilityOBFEfficacy} simulates a series of three-arm trials with an interim analysis with a stopping rule for futility and efficacy based on O'Brien-Fleming spending function.
#' 
...
SimulateThreeArmOBFFutilityOBFEfficacy  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  end.futility.s1.trial <- numeric()  # Vector of whether the trial stopped at interim for futility
  end.efficacy.s1.trial <- numeric()  # Vector of whether the trial stopped at interim for efficacy
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      # The trial stops for futility (this is a non-binding rule) if both hypotheses are retained
      # In the interim analysis one has to retain all individual hypotheses for which there exists an intersection hypothesis which is retained with its combination test
      # If both hypotheses are retained, the trial stops and the control treatment is carried over to the next trial
      # If at least one hypothesis is not retained, the treatment with the smallest p-value is carried over to the second stage
      # The final conclusion is based on the combination function "as usual"
      #install.packages('mppa')
      #library('mppa')
      #pvalue.futility.ia <- 0.5
      #pvalue1.s1         <- 0.6
      #pvalue2.s1         <- 0.24
      #pvalue12.s1        <- SimesFirstStage(t(t(c(pvalue1.s1, pvalue2.s1))))
      #pvalue12.s1
      #simes.test(c(pvalue1.s1, pvalue2.s1))
      #(pvalue1.s1 >= pvalue.futility.ia) 
      #(pvalue2.s1 >= pvalue.futility.ia)
      #(pvalue12.s1 >= pvalue.futility.ia)
      #((pvalue1.s1 >= pvalue.futility.ia) & (pvalue2.s1 >= pvalue.futility.ia)) |
      #((pvalue1.s1 >= pvalue.futility.ia) & (pvalue12.s1 >= pvalue.futility.ia)) |
      #((pvalue2.s1 >= pvalue.futility.ia) & (pvalue12.s1 >= pvalue.futility.ia)) |
      #((pvalue12.s1 >= pvalue.futility.ia))
      end.futility.s1         <- ifelse((((pvalue1.s1 >= pvalue.futility.ia) & (pvalue2.s1 >= pvalue.futility.ia)) |
                                           ((pvalue1.s1 >= pvalue.futility.ia) & (pvalue12.s1 >= pvalue.futility.ia)) |
                                           ((pvalue2.s1 >= pvalue.futility.ia) & (pvalue12.s1 >= pvalue.futility.ia)) |
                                           ((pvalue12.s1 >= pvalue.futility.ia))), 1, 0)  
      # ----- IF THE TRIAL STOPPED AT INTERIM (FOR FUTILITY)  -----        
      lambda.futility.s1     <- lambdaC
      lambda.obs.futility.s1 <- lambdaC.obs.s1
      
      # The trial stops for efficacy if (at least) one hypothesis is rejected
      # In the interim analysis, one has to reject all individual hypotheses for which all intersection hypotheses are rejected with its combination test
      # If any hypothesis is rejected (one of them or both), the trial stops and the experimental treatment with the smallest p-value is carried over to the next trial
      # If none of the two hypotheses is rejected, the treatment with the smallest p-value is carried over to the second stage
      # The final conclusion is based on the combination function "as usual"
      #install.packages('mppa')
      #library('mppa')
      #pvalue.efficacy.ia <- 0.025
      #pvalue1.s1         <- 0.013
      #pvalue2.s1         <- 0.6
      #pvalue12.s1        <- SimesFirstStage(t(t(c(pvalue1.s1, pvalue2.s1))))
      #pvalue12.s1
      #simes.test(c(pvalue1.s1, pvalue2.s1))
      #(pvalue1.s1 <= pvalue.efficacy.ia)
      #(pvalue2.s1 <= pvalue.efficacy.ia)
      #((pvalue1.s1 <= pvalue.efficacy.ia) | (pvalue2.s1 <= pvalue.efficacy.ia))
      #(pvalue12.s1 <= pvalue.efficacy.ia) 
      #((pvalue1.s1 <= pvalue.efficacy.ia) | (pvalue2.s1 <= pvalue.efficacy.ia)) &
      #(pvalue12.s1 <= pvalue.efficacy.ia)
      end.efficacy.s1         <- ifelse((((pvalue1.s1 <= pvalue.efficacy.ia) | (pvalue2.s1 <= pvalue.efficacy.ia)) &
                                           (pvalue12.s1 <= pvalue.efficacy.ia)), 1, 0)  
      # ----- IF THE TRIAL STOPPED AT INTERIM (FOR EFFICACY)  -----        
      lambda.efficacy.s1      <- lambdaE[which(select.s1.ind!=0)]
      lambdaE.obs.s1          <- c(lambdaE1.obs.s1, lambdaE2.obs.s1)
      lambda.obs.efficacy.s1  <- lambdaE.obs.s1[which(select.s1.ind!=0)]
      
      # ----- IF THE TRIAL STOPPED AT INTERIM -----        
      end.s1        <- end.futility.s1 + end.efficacy.s1  # The trial stops whether for futility or efficacy
      
      # If the trial stops for futility, retain futility results, if the trial stops for efficacy, retain efficacy results
      lambda.obs.s1 <- ifelse(end.futility.s1 == 1, lambda.obs.futility.s1, lambda.obs.efficacy.s1)
      lambda.s1     <- ifelse(end.futility.s1 == 1, lambda.futility.s1, lambda.efficacy.s1)
      ...
      end.futility.s1.trial    <- c(end.futility.s1.trial, end.futility.s1)
      end.efficacy.s1.trial    <- c(end.efficacy.s1.trial, end.efficacy.s1)
      ...
      pts.year.E.s2 <- sum(survival.data.s2[which(survival.data.s2$group.s2 != 2), "del.s2"])  # Sum of patient-years in the (carried over) experimental group at the end of the second stage
  ...
  results   <- cbind.data.frame(accrual.rate.trial,
                                ...
                                end.futility.s1.trial,
                                end.efficacy.s1.trial,
                                time.end.trial,
                                end.s2.trial,
                                exp.retained.trial,
...
results <- SimulateThreeArmOBFFutilityOBFEfficacy(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
