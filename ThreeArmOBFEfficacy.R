#' @title Simulation of a series of three-arm trials with an interim analysis with a stopping rule for efficacy based on O'Brien-Fleming spending function.
#' @author Mohamed Amine BAYAR
#' \code{SimulateThreeArmOBFEfficacy} simulates a series of three-arm trials with an interim analysis with a stopping rule for efficacy based on O'Brien-Fleming spending function.
#' 
...
SimulateThreeArmOBFEfficacy  <- function(accrual, alpha, beta, hr, distrib, baseline, FU, nbrepeat, Tmax){  # ----- START OF SIMUL FUNCTION -----
  ...
  for (i in 1:nbrepeat){  # ----- START LOOP 10 000 REPETITIONS -----
    ...
    while (ifelse(is.na(time < Tmax), FALSE,  time < Tmax)){  # ----- START SERIES -----
      ...
      #((pvalue1.s1 <= pvalue.efficacy.ia) | (pvalue2.s1 <= pvalue.efficacy.ia)) &
      #(pvalue12.s1 <= pvalue.efficacy.ia)
      end.s1         <- ifelse((((pvalue1.s1 <= pvalue.efficacy.ia) | (pvalue2.s1 <= pvalue.efficacy.ia)) &
      ...
      lambda.s1      <- lambdaE[which(select.s1.ind!=0)]
      lambdaE.obs.s1 <- c(lambdaE1.obs.s1, lambdaE2.obs.s1)
      lambda.obs.s1  <- lambdaE.obs.s1[which(select.s1.ind!=0)]
...
results <- SimulateThreeArmOBFEfficacy(
  accrual  = 100,
  alpha    = 0.5,
  beta     = 0.1,
  hr       = 0.75,
